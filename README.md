...
- 個別ファイルをローカル保存 → Vault APIアップロード: `20-30.wav` → `20-30.json`
...
### v4.0.0 (2025-06-28)
- **Vault API連携強化**: Whisper APIと同様のローカル保存→Vaultアップロード方式
- **統合ファイル生成無効化**: 個別JSONファイルのみ生成・アップロード
- **ファイル管理改善**: `/opensmile/` ディレクトリでの統一管理
- **デモアカウント対応**: user123での動作確認完了
